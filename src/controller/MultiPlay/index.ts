    ...
    onPlayerChange &&
      this.socket?.on("PLAYER_CHANGE", (res) => {
        ...
        this.controller.ui.menu.setNotify(
          `${userName} ${
            action === "join" ? language.wsMessage.PLAYER_CHANGE_JOIN : language.wsMessage.PLAYER_CHANGE_LEAVE
          }`
        );
        onPlayerChange(res);
      });
  ...
  bindGame() {
    ...
    this.socket?.on("LOG_UPDATE", (res) => {
      ...
      this.playersController.update(
        userName,
        new THREE.Vector3(position.posX, position.posY, position.posZ),
        new THREE.Euler(position.dirX, position.dirY, position.dirZ, "YXZ")
      );
    });
    this.socket?.on("ROOM_DISSOLVE", () => {
      this.controller.ui.menu.setNotify(
        language.wsMessage.ROOM_DISSOLVED,
        1000,
        this.controller.uiController.ui.actionControl.elem
      );
      this.clear();
