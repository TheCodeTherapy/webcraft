...
class PlayersController {
  ...
  update(name: string, pos: THREE.Vector3, reward: THREE.Euler) {
    this.players.forEach((d) => {
      if (!d) return;
      if (
        Math.abs(d.position.x - config.state.posX) > config.renderer.stageSize / 2 ||
        Math.abs(d.position.z - config.state.posZ) > config.renderer.stageSize / 2
      )
  ...
  init(roomId, scene: THREE.Scene, playerNames: string[]) {
    ...
    this.scene = scene;
    this.players = playerNames.map((d, i) =>
      d === null
        ? null
        : new Player({
            idx: (st + i) % skinsMap.length,
            pos: new THREE.Vector3(0, 0, 0),
            reward: new THREE.Euler(0, 0, 0, "YXZ")
          })
    );
