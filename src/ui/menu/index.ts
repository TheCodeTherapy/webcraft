...
import { config, defaultConfig, language, languages } from "../../core/_config";
...
class Menu {
  ...
  toAboutMenu({ back }) {
    ...
    this.boxElem.innerHTML = `
			...
			<div class="box-line color-white">
			${language.aboutItems[2]}<a href="https://github.com/KairuiLiu/ThreeCraft" target="blank">Github</a>${
      language.aboutItems[3]
        ...
        } <span class="with-love">❤</span>&nbspLiu Kairui
			</div>
			<br/>
			<button class="button" id="backMenu">${language.backMenu}</button>`;
    const backMenu = document.getElementById("backMenu");
