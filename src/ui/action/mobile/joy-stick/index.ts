...
class JoyStick {
  ...
  emitFun: (arg0: unknown) => void;

  constructor(el: HTMLElement, jConfig: { width: number; height: number }, emitFun: (arg0: unknown) => void) {
    this.el = el;
    this.enable = true;
  ...
  drawInternal() {
    ...
    if (this.movedX + this.internalRadius > this.canvas.width)
      this.movedX = this.canvas.width - (this.internalRadius + 10);
    ...
    if (this.movedY + this.internalRadius > this.canvas.height)
      this.movedY = this.canvas.height - this.internalRadius - 10;
