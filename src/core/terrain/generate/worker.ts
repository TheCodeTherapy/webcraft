    ...
    for (let curZ = stz; curZ < edz; curZ += fragmentSize) {
          ...
          if (y < horizonHeight) {
            ...
            if (y > treeBaseHeight) {
              ...
              const treeHeight = Math.floor(
                noiseGen.noise(i / treeSeedGap, j / treeSeedGap, treeSeed) * maxHeight * 1.5
              );
                ...
                for (let leaveX = stxL; leaveX <= edxL; leaveX += 1) {
                  for (let leaveZ = stzL; leaveZ <= edzL; leaveZ += 1) {
                    const deltaY = Math.max(
                      Math.abs(
                        Math.floor(
                          (noiseGen.noise(leaveX / treeSeedGap / 10, leaveZ / treeSeedGap / 10, treeSeed) * maxHeight) /
                            2
                        )
                      ),
                      2
      ...
      log.forEach((d) => {
        ...
        if (d.type === null || blockFragment.idMap.has(`${d.posX}_${d.posY}_${d.posZ}`)) {
          ...
          if (!block || block.typeIdx === undefined || block.idx === undefined) return;
