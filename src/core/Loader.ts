...
// Import block icons - must be manually imported to prevent Vite from ignoring dependencies
...
// Disable texture blurring for all materials
[
  acaciaLeavesTexture,
  andesiteTexture,
  bedrockTexture,
  ...
  cyberSkinTexture,
  plusSkinTexture
].forEach((d) => (d.magFilter = NearestFilter));

// Import block break sound effects
...
// Import block step sound effects
...
export const blockLoader: Record<string, BlockProperties> = {
  acaciaLeaves: {
    name: "acaciaLeaves",
  ...
  ice: {
    ...
    material: new MeshStandardMaterial({ map: iceTexture, color: 0x00698e, transparent: true, opacity: 0.3 })
  },
  blockIron: {
    name: "blockIron",
    block3d: blockIron3d,
  ...
  water: {
    ...
    material: new MeshStandardMaterial({ map: waterTexture, color: 0x00799e, transparent: true, opacity: 0.3 }),
    accessible: true,
    break: waterBreak
  },
  acaciaWood: {
...
// Generate materials for blocks that don't have them defined
blockTypes.forEach((blockType) => {
  const block = blockLoader[blockType as keyof typeof blockLoader] as BlockProperties;

  if (!block.material) {
    const isTextureArray = block.textureImg instanceof Array;
    const textures = block.textureImg as Texture | Texture[];

    block.material = isTextureArray
      ? (textures as Texture[]).map((texture) => new MeshStandardMaterial({ map: texture }))
      : new MeshStandardMaterial({
          map: textures as Texture
        });
  }
});

// Generate sound effects for blocks
blockTypes.forEach(
  (d) =>
    (blockLoader[d as keyof typeof blockLoader] as BlockProperties).step ||
    ((blockLoader[d as keyof typeof blockLoader] as BlockProperties).step = stoneStep)
);
blockTypes.forEach(
  (d) =>
    (blockLoader[d as keyof typeof blockLoader] as BlockProperties).break ||
    ((blockLoader[d as keyof typeof blockLoader] as BlockProperties).break = netherBreak)
);
